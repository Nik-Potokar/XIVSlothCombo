            ...
            if (actionID == DRK.Souleater)
            {
                ...
                if (incombat)
                {
                    ...
                    if (!inOpener)
                    {
                        ...
                        {
                            //Mana Features
                            if (IsEnabled(CustomComboPreset.DarkManaOvercapFeature) && CanWeave(actionID,0.2))
                            {
                        ...
                        if (level >= DRK.Levels.Delirium && IsEnabled(CustomComboPreset.DeliriumFeature) && IsEnabled(CustomComboPreset.DarkMainComboCDsGroup))
                        {
                            ...
                            if (IsEnabled(CustomComboPreset.DarkDeliriumOnCD) && ((gauge.Blood >= 50 && GetCooldownRemainingTime(DRK.Delirium) < 6 && GetCooldownRemainingTime(DRK.Delirium) > 0) || (IsOffCooldown(DRK.Delirium) && gauge.Blood >=50)))
                                return DRK.Bloodspiller;
                        }
