        ...
        internal class BlackAoEComboFeature : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                if (actionID == Flare)
                {
                    ...
                    var polyToStore = Service.Configuration.GetCustomIntValue(Config.BlmPolygotsStored);
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                if (actionID == Scathe)
                {
                    ...
                    if (IsEnabled(CustomComboPreset.BlackSimpleOpenerFeature) && level >= Levels.Foul)
                    {
                        ...
                        if ((!inOpener || (openerFinished && CanSpellWeave(actionID))) && !HasEffect(Buffs.Sharpcast) && (GetRemainingCharges(Sharpcast) >= 1))
                        {
                            return Sharpcast;
                        }
                        ...
                        if (inCombat && inOpener && !openerFinished)
                        {
                            ...
                            {
                                // First Triplecast
                                if (lastComboMove != Triplecast && !HasEffect(Buffs.Triplecast) && GetRemainingCharges(Triplecast) == 2)
                                {
                    ...
                    if (gauge.ElementTimeRemaining > 0)
                    {
                        ...
                        // Thunder uptime
                        if (gauge.ElementTimeRemaining >= 6000 && currentMP >= MP.AspectThunder)
                        {
                            if (level < Levels.Thunder3 && lastComboMove != Thunder && thunderRecast(4) && !TargetHasEffect(Debuffs.Thunder2))
                            {
                                return Thunder;
                            }
                        ...
                        if (canWeave)
                        {
                            ...
                            if (IsEnabled(CustomComboPreset.BlackSimpleBuffsFeature))
                            {
                                ...
                                if (level >= Levels.Sharpcast && GetRemainingCharges(Sharpcast) > 0 && !HasEffect(Buffs.Sharpcast))
                                {
                                    return Sharpcast;
                                }
                            }
                    ...
                    if (gauge.PolyglotStacks > 0 && gauge.ElementTimeRemaining >= 5000 && (gauge.InUmbralIce || (gauge.InAstralFire && gauge.UmbralHearts == 0)))
                    {
                        if (level >= Levels.Xenoglossy)
                        {
                            ...
                            if (gauge.PolyglotStacks == 2 && GetCooldown(LeyLines).CooldownRemaining >= 30 && GetCooldown(Triplecast).ChargeCooldownRemaining >= 30)
                            {
        ...
        internal class BlackSimpleTranposeFeature : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                if (actionID == Scathe)
                {
                    ...
                    // Only enable sharpcast if it's available
                    if ((!inOpener || (openerFinished && canWeave)) && !HasEffect(Buffs.Sharpcast) && (GetRemainingCharges(Sharpcast) >= 1))
                    {
                        return Sharpcast;
                    }
