...
using static XIVSlothCombo.Combos.JobHelpers.SGE;
...
internal static class SGE
{
    ...
    internal class SGE_Rhizo : CustomCombo
    {
        ...
        protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
        {
            return AddersgallList.Contains(actionID) && ActionReady(Rhizomata) && !HasAddersgall(Gauge) &&
    ...
    internal class SGE_DruoTauro : CustomCombo
    {
        ...
        protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
        {
            return actionID is Taurochole && IsOnCooldown(Taurochole)
                ? Druochole
                : actionID;
        }
    }
    ...
    internal class SGE_AoE_DPS : CustomCombo
    {
        ...
        protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
        {
            if (DyskrasiaList.Contains(actionID) && !HasEffect(Buffs.Eukrasia))
            {
                // Variant Rampart
    ...
    internal class SGE_ST_DPS : CustomCombo
    {
        ...
        protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
        {
            ...
            if (ActionFound)
            {
                ...
                if (HasBattleTarget() && !HasEffect(Buffs.Eukrasia))
                    ...
                    if (IsEnabled(CustomComboPreset.SGE_ST_DPS_EDosis) && LevelChecked(Eukrasia) && InCombat())
                        ...
                        if (DosisList.TryGetValue(OriginalHook(actionID), out ushort dotDebuffID))
                        {
                            ...
                            float refreshtimer = Config.SGE_ST_DPS_EDosis_Adv ? Config.SGE_ST_DPS_EDosisThreshold : 6;
                ...
                {
                    // Toxikon
                    if (Config.SGE_ST_DPS_Movement[0] && LevelChecked(Toxikon) && HasAddersting(Gauge))
        ...
        protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
        {
            if (actionID is Diagnosis)
            {
                ...
                foreach (int prio in Config.SGE_ST_Heals_Priority.Items.OrderBy(x => x))
                {
                    ...
                    int config = SGEHelper.GetMatchingConfigST(index, out uint spell, out bool enabled);
