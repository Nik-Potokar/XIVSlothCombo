                    ...
                    if (inCombat && !inOpener)
                    {
                        if (IsEnabled(CustomComboPreset.MNK_ST_Simple_CDs))
                        {
                            ...
                            if (canDelayedWeave)
                            {
                                ...
                                if (LevelChecked(All.TrueNorth) && IsEnabled(CustomComboPreset.MNK_TrueNorthDynamic) && GetRemainingCharges(All.TrueNorth) > 0 && !HasEffect(All.Buffs.TrueNorth) && LevelChecked(Demolish) && HasEffect(Buffs.CoerlForm) && TargetNeedsPositionals())
                                {
