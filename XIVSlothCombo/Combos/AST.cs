...
namespace XIVSlothComboPlugin.Combos
{
    internal static class AST
    {
        ...
        /*internal class AstrologianDpsAoEFeature : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                if ((actionID is Gravity or Gravity2) && InCombat())
                {
                    if (IsEnabled(CustomComboPreset.AstrologianLightSpeedFeature) &&
                        level >= Levels.Lightspeed &&
                        IsOffCooldown(Lightspeed) &&
                        lastComboMove == OriginalHook(actionID) && //Is this needed
                        CanSpellWeave(actionID)
                        ) return Lightspeed;

                    if (IsEnabled(CustomComboPreset.AstrologianAstrodyneFeature) &&
                        level >= Levels.Astrodyne &&
                        !GetJobGauge<ASTGauge>().ContainsSeal(SealType.NONE) && //GetJobGauge<ASTGauge>().Seals.Count == 3 Would this be faster/better instead of checking each seal for NONE?
                        CanSpellWeave(actionID)
                        ) return Astrodyne;
                    ...
                    if (IsEnabled(CustomComboPreset.AST_ST_DPS_LucidOption) &&
                        level >= All.Levels.LucidDreaming &&
                        IsOffCooldown(All.LucidDreaming) &&
                        LocalPlayer.CurrentMp <= GetOptionValue(Config.ASTLucidDreamingFeature) &&
                        CanSpellWeave(actionID)
        ...
        /*internal class AstrologianAlternateDpsFeature : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                if (actionID == Combust1 || actionID == Combust2 || actionID == Combust3)
                {
                    ...
                    if (IsEnabled(CustomComboPreset.AST_DPS_LucidOption) && level >= All.Levels.LucidDreaming)
                    {
