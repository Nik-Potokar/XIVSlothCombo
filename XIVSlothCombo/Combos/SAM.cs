                            ...
                            if (comboTime > 0)
                            {
                                if (lastComboMove == Hakaze && level >= Levels.Jinpu)
                                {
                                    if (IsEnabled(CustomComboPreset.YukionST) && gauge.Sen.HasFlag(Sen.SETSU) == false && level >= Levels.Yukikaze && HasEffect(Buffs.Fugetsu) && HasEffect(Buffs.Fuka))
                                        return Yukikaze;
                                    if ((level < Levels.Kasha && ((GetBuffRemainingTime(Buffs.Fugetsu) < GetBuffRemainingTime(Buffs.Fuka)) || !HasEffect(Buffs.Fugetsu))) || (level >= Levels.Gekko && gauge.Sen.HasFlag(Sen.GETSU) == false))
                                        return Jinpu;
                                    if (IsEnabled(CustomComboPreset.KashaonST) &&
                                        ((level < Levels.Gekko && ((GetBuffRemainingTime(Buffs.Fuka) < GetBuffRemainingTime(Buffs.Fugetsu)) || !HasEffect(Buffs.Fuka))) || (level >= Levels.Kasha && gauge.Sen.HasFlag(Sen.KA) == false)))
                                        return Shifu;
                                    return Jinpu;
                                }
