...
internal class BLMOpenerLogic : BLM
{
    ...
    private bool DoOpener(ref uint actionID)
    {
        ...
        if (currentState == OpenerState.InOpener)
        {
            ...
            if (((actionID == Triplecast && GetRemainingCharges(Triplecast) < 2) ||
                 (actionID == Amplifier && IsOnCooldown(Amplifier)) ||
                 (actionID == LeyLines && IsOnCooldown(LeyLines)) ||
                 (actionID == All.LucidDreaming && IsOnCooldown(All.LucidDreaming)) ||
                 (actionID == Manafont && IsOnCooldown(Manafont)) ||
                 (actionID == All.Swiftcast && IsOnCooldown(All.Swiftcast)) ||
                 (actionID == Xenoglossy && Svc.Gauges.Get<BLMGauge>().PolyglotStacks < 1)) &&
